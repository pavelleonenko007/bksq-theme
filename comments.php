...
<div class="comments">
  <div class="comments-title"><?php comments_number(); ?>
  </div>
...
function mytheme_comment($comment, $args, $depth)
{
    ...
    case '':
?>
<div <?php if ($GLOBALS['comment_depth'] !== 1) {
    echo comment_class('children', '', '', false);
} else {
    echo comment_class('', '', '', false);
} ?> id="comment-<?php comment_ID() ?>" >
  <div class="comment-body" id="comment-<?php comment_ID(); ?>">
    <?php echo get_avatar($comment->comment_author_email, $args['avatar_size']); ?>
    <div class="comment-author"><?php printf(__('%s'), get_comment_author_link()) ?>
    </div>
    <a class="comment-date"
      href="<?php echo esc_url(get_comment_link($comment->comment_ID)) ?>"><?php printf(__('%1$s at %2$s'), get_comment_date(), get_comment_time()) ?></a>
