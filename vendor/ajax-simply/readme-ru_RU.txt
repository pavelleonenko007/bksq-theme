...
Документацию смотрите [по этой ссылке](https://wp-kama.ru/?p=9054).
...
= 1.5.3 =
* NEW: Добавил `[type="submit"]:focus` к селекторам выбора полей формы для сбора данных.

= 1.5.2 =
* IMP: Добавил `:focus` к селектору `button`. См. релиз 1.5.1.

= 1.5.1 =
* NEW: Добавлен тег `<button>` при сборе данных формы (наровне с input, select).

= 1.5.0 =
* FIX: Правильный вызов события `ajaxs_done` для `ajaxs(...).done(func)`. Тоже самое для событий `ajaxs_fail`, `ajaxs_always`. Например, при `ajaxs(...).done(func)` сначала срабатывало событие `ajaxs_done` затем функция `func`, а должно быть наоборот.
...
= 1.4.9 =
* FIX: баг связанный с последнем релизом.

= 1.4.8 =
* NEW: Добавьте HTML элемент `<div id="#ajaxs-console">` или `<div class=".ajaxs-console>`, чтобы получать вывод консоли на странице, например для показа PHP ошибок.

= 1.4.5 =
* NEW: Параметр `skip_basic_nonce` - `ajaxs( 'funcname', { skip_basic_nonce:1 } )`. Позволяет пропустить базовую проверку nonce кода для отдельного запроса, когда такая проверка включена в настройках.

= 1.4.4 =
* NEW: Новый метод `$jx->jscode()` алиас `$jx->jseval()`.

= 1.4.3 =
* NEW: Третий параметр для метода `$jx->html( $selector, $html, $method )`. $method позволяет указать как именно нужно использовать передаваемый HTML по отношению к указанному селектору. Например, теперь можно дополнить имеющийся HTML селектора указав 'append' в третьем параметре. Возможные значения параметра: html(по умолчанию)|append|prepend|replace|before|after.
* NEW: Методы: `$jx->append()`, `$jx->prepend()`, `$jx->before()`, `$jx->after()`, `$jx->replace()`.

= 1.4.2 =
* IMPROVE: Группировка PHP ошибок в консоли.

= 1.4.1 =
* BUG: Невозможно было указать 0 в качестве последнего параметра в фукнцию: `$jx->log( 'foo', 0 )`.

= 1.4.0 =
* NEW: функция `doing_ajaxs()` позволяет определять выполняется ли в текущий момент запрос Ajax Simply.
* NEW: Код частично переписан с исползованием плюшек PHP 5.6. Теперь минимальные требования PHP 5.6.
* NEW: `console.group()` - теперь можно группирвоать вызовы console.log(), для этого укажите сразу несколько параметров в метод `jx::log()`: `$jx->log( one, two, three )`.

= 1.3.9 =
* NEW: Метод `jx::refresh()` - алиас метода `jx::reload()`.

= 1.3.8 =
* FIX: Если нет input полей с attr "name", будет искать input c атрибутом "id" или "data-name" в качестве имени.
