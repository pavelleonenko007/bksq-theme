...
final class AJAXS_options_page {
	...
	static function init(){
		...
		add_action( 'network_admin_menu', [ __CLASS__, 'add_submenu_page' ] );
		add_action( 'admin_menu', [ __CLASS__, 'add_submenu_page' ], 9999 );

		## Link on the plugins page
		add_filter( 'network_admin_plugin_action_links', [ __CLASS__, 'plugin_action_links' ], 10, 2 );
	...
	static function add_submenu_page(){
		...
		load_plugin_textdomain( 'jxs', false, basename(AJAXS_PATH) .'/lang/' );

		// баг - страница плагина не работает если в списке submenu есть `options.php`.
		// изменим её название (без последствий).
		// Текущая страница обрабатывается файлом `options.php`
		global $submenu;
		foreach( $submenu as $key => $elems ){
			foreach( $elems as $index => $elem ){
				if( 'options.php' === $elem[2] )
					$submenu[ $key ][ $index ][2] = 'options.php?foo';
			}
		}

		add_submenu_page( null, '', '', 'manage_options', 'ajaxs_opt_page', [ __CLASS__, 'options_page_out' ] );
	}
	...
	static function options_page_out(){
		...
		$id = 'allow_nonce';
		add_settings_field( $id, __('Default nonce check','jxs'), [ __CLASS__, 'opt_field' ], 'ajaxs_opt_section', 'ajxs_section', [
			'id'    => $id,
		...
		$id = 'use_inline_js';
		add_settings_field( $id, __('Use inline js','jxs'), [ __CLASS__, 'opt_field' ], 'ajaxs_opt_section', 'ajxs_section', [
			'id'    => $id,
			'title' => __('Include js into HTML code, but don`t connect it as a file.','jxs'),
		] );

		$id = 'front_request_file';
		add_settings_field( $id, __('Front request file','jxs'), [ __CLASS__, 'opt_field' ], 'ajaxs_opt_section', 'ajxs_section', [
			'id'    => $id,
			'title' => __('Use another file for front requests (not admin-ajax.php).','jxs'),
		] );

		$id = 'post_max_size';
		add_settings_field( $id, __('Max upload file and POST request sizes','jxs'), [ __CLASS__, 'opt_field' ], 'ajaxs_opt_section', 'ajxs_section', [
			'id'    => $id,
		] );
	...
	static function ajaxs_save_options( $jx ){
		...
		foreach( $opts as $key => & $val ){
			if( in_array( $key, [ 'allow_nonce', 'use_inline_js', 'front_request_file' ], true ) )
				$val = !! $val;
			else
		...
		if( is_multisite() )
			$done = update_site_option( AJAXS_OPTNAME, $opts ); // для обычного сайта ставит $autoload = no
		else
			$done = update_option( AJAXS_OPTNAME, $opts );
